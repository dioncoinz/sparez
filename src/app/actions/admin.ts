...
export async function inviteUser(_prev:ActionState,formData:FormData):Promise<ActionState>{
  ...
  try{
    ...
    const redirectTo=`${await invitationOrigin()}/auth/callback`;
    let userId:string|undefined;
    const{data,error}=await admin.auth.admin.inviteUserByEmail(email,{redirectTo,data:{invited_to_organisation:membership.organisation_id}});
    if(error){
      ...
      if(!userId)return{error:"This account exists but could not be linked automatically. Contact support."};
      const{error:recoveryError}=await admin.auth.resetPasswordForEmail(email,{redirectTo});
      if(recoveryError)return{error:`Access was restored, but the setup email could not be sent: ${recoveryError.message}`};
    }else userId=data.user.id;
    await admin.from("profiles").upsert({id:userId,email},{onConflict:"id"});
    const{error:memberError}=await admin.from("memberships").upsert({organisation_id:membership.organisation_id,user_id:userId,role,is_active:true,invited_by:membership.user_id},{onConflict:"organisation_id,user_id"});

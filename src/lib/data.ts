...
export async function getItem(id: string): Promise<Item | null> {
  const supabase = await createClient();
  const { data } = await supabase.from("items").select("*, creator:profiles!items_created_by_fkey(id,full_name,email), item_photos(*), item_movements(*, remover:profiles!item_movements_removed_by_fkey(id,full_name,email), reverser:profiles!item_movements_reversed_by_fkey(id,full_name,email))").eq("id", id).maybeSingle();
  if (!data) return null;

...
import { LogOut, Menu, Settings, Upload, Users, X } from "lucide-react";
...
export function AppShell({ membership, children }: { membership: Membership; children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-canvas pb-32 lg:pb-0">
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur lg:hidden"><Logo compact /><div className="text-center"><p className="text-sm font-bold text-ink">Sparez</p><p className="max-w-48 truncate text-[11px] text-slate-500">{membership.organisation?.name}</p></div><details className="group relative"><summary className="grid h-11 w-11 cursor-pointer list-none place-items-center rounded-xl border border-slate-200 text-slate-600 [&::-webkit-details-marker]:hidden"><Menu className="h-5 w-5 group-open:hidden"/><X className="hidden h-5 w-5 group-open:block"/></summary><div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-2 shadow-xl"><p className="truncate px-3 py-2 text-xs text-slate-500">{name}</p>{membership.role==="admin"&&<div className="mb-1 border-b border-slate-100 pb-1"><p className="px-3 pb-1 pt-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">Administration</p><Link href="/users" className="flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"><Users className="h-4 w-4 text-forest-700"/>Users</Link><Link href="/export" className="flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"><Upload className="h-4 w-4 text-forest-700"/>Export</Link><Link href="/settings" className="flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"><Settings className="h-4 w-4 text-forest-700"/>Settings</Link></div>}<form action={signOut}><button className="flex min-h-11 w-full items-center gap-3 rounded-lg px-3 text-sm font-semibold text-slate-700 hover:bg-slate-50"><LogOut className="h-4 w-4" /> Sign out</button></form></div></details></header>
